...
mappingFileInput.addEventListener('change', async () => {
    ...
    if (!file) {
        mappingData = null;
        mappingStatus.textContent = 'No mapping loaded.';
        updateButtonState();
        return;
    }
    ...
    if (mappingData) {
        mappingStatus.textContent = `Mapping loaded: ${mappingData.rows.length} rows.`;
    } else {
        mappingStatus.textContent = 'Failed to load mapping. Please upload mapping.csv.';
    }
    updateButtonState();
});
...
convertButton.addEventListener('click', async () => {
    ...
    downloadLink.href = URL.createObjectURL(new Blob([result.csv], { type: 'text/csv;charset=utf-8;' }));
    downloadLink.download = `converted-${direction}.csv`;
...
function parseMappingCsv(csvText) {
    ...
    const required = [
        'DATATYPE', 'PRODUCT', 'ITEM1', 'ITEM2',
        'ENERGY_PRODUCT', 'MAIN_FLOW', 'FLOW_BREAKDOWN',
        'PLANT_TYPE', 'STOCKS', 'VIS_A_VIS_AREA',
        'MEASURE_VALUE_TYPE', 'UNIT_MEASURE'
    ];

    const headerIndex = {};
    header.forEach((cell, index) => { headerIndex[cell] = index; });
...
function convertSdmxToEvo(text, mappingData) {
    ...
    if (rows.length < 2) {
        throw new Error('SDMX input file must contain a header row and at least one record.');
    }

    const header = rows[0].map(cell => cell.trim().toUpperCase());
    const idx = arrayToIndex(header);
    const required = [
        'QUEST_SOURCE', 'REF_AREA', 'TIME_PERIOD',
        'ENERGY_PRODUCT', 'MAIN_FLOW', 'FLOW_BREAKDOWN',
        'PLANT_TYPE', 'STOCKS', 'VIS_A_VIS_AREA',
        'MEASURE_VALUE_TYPE', 'UNIT_MEASURE', 'OBS_VALUE', 'OBS_STATUS'
    ];
    ...
    const outputRows = [
        ['COUNTRY', 'QUEST', 'DATATYPE', 'PRODUCT', 'ITEM1', 'ITEM2', 'TIME', 'VALUE', 'FLAG']
    ];
    ...
    for (let i = 1; i < rows.length; i += 1) {
        ...
        const key = buildSdmxKey(energyProduct, mainFlow, flowBreakdown, plantType, stocks, visAVisArea, measureValueType, unitMeasure);
...
function convertEvoToSdmx(text, mappingData) {
    const rows = parseCsv(text, ',');
    if (rows.length < 2) {
        throw new Error('EVO input file must contain a header row and at least one record.');
    }

    const header = rows[0].map(cell => cell.trim().toUpperCase());
    const idx = arrayToIndex(header);
    const required = ['COUNTRY', 'QUEST', 'DATATYPE', 'PRODUCT', 'ITEM1', 'ITEM2', 'TIME', 'VALUE', 'FLAG'];
    required.forEach(name => {
        if (!(name in idx)) {
    ...
    const outputRows = [[
        'DATAFLOW', 'QUEST_SOURCE', 'REF_AREA', 'FREQ',
        'ENERGY_PRODUCT', 'MAIN_FLOW', 'FLOW_BREAKDOWN',
        'PLANT_TECH', 'PLANT_TYPE', 'STOCKS', 'INFRASTRUCTURE_IND',
        'VIS_A_VIS_AREA', 'MEASURE_VALUE_TYPE', 'FACILITY_ID',
        'TIME_PERIOD', 'OBS_VALUE', 'UNIT_MEASURE', 'OBS_STATUS',
        'CONF_STATUS', 'COMMENT_OBS', 'FACILITY_TYPE', 'FACILITY_NAME'
    ]];
...
function buildEvoToSdmxMap(rows) {
    const map = {};
    rows.forEach(item => {
        const key = buildEvoKey(item.datatype, item.product, item.item1, item.item2);
        map[key] = item;
    });
    return map;
}
...
function buildSdmxKey(energyProduct, mainFlow, flowBreakdown, plantType, stocks, visAVisArea, measureValueType, unitMeasure) {
    return [energyProduct, mainFlow, flowBreakdown, plantType, stocks, visAVisArea, measureValueType, unitMeasure].map(v => v.trim()).join('|');
}
